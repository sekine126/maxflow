...
db_name = params["d"]
link_date = params["f"]
link_filename = "./data/crawl/#{db_name}_crawl_#{link_date}.txt"
nodes = []
links = []
# 初期データ
...
# 計算結果用のIDリストをファイルに出力
file = File.open("./data/matrix/ids_#{params["d"]}_#{params["f"]}.txt", "w")
...
# 計算結果の隣接行列をファイルに出力
file = File.open("./data/matrix/#{params["d"]}_#{params["f"]}.txt", "w")
matrix.each { |row| 
  line = ""
  row.each { |v| 
    line = line + "#{v} "
