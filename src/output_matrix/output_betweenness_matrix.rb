...
link1_date = params["f"]
link2_date = params["t"]
link_filename = "./data/update/bet_#{db_name}_#{link1_date}_#{link2_date}.txt"
nodes = []
links = []
# 初期データ
...
# 計算結果用のIDリストをファイルに出力
file = File.open("./data/matrix/bet_ids_#{params["d"]}_#{params["t"]}.txt", "w")
...
# 計算結果の隣接行列をファイルに出力
file = File.open("./data/matrix/bet_#{params["d"]}_#{params["t"]}.txt", "w")
matrix.each { |row| 
  line = ""
  row.each { |v| 
    line = line + "#{v} "
